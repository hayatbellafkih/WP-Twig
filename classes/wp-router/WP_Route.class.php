...
class WP_Route extends WP_Router_Utility {
	 ...
	 * @throws Exception
	 * @param string $id
	 *        	A unique string used to refer to this route
	 * @param array $properties
	 *        	An array of key/value pairs used to set
	...
	public function execute(WP $query) {
		...
		//$page_contents="";
		// how add content ? twig or php
		if ($this->is_twig) {
			try {
				$twig = TWP_Manager::getEnvirenement ();
				/* include functions who pass data to Twig templates */
				$template = $twig->loadTemplate ( $this->twig_template );
				$tab_function = array ();
				foreach ( $this->twig_data as $function ) {
					if (is_callable ( $function ) && is_array ( call_user_func ( $function ) )) {
						$tab_function = array_merge ( $tab_function, call_user_func ( $function ) );
					}
				}
				$page_contents = $template->render ( $tab_function );
			} catch ( Twig_Error_Syntax $e ) {
				echo "lexception" . $e->getRawMessage () . "<br>";
				TWP_Manager::displayTemplate ( 'error_template.twig', array (
						'error' => $e 
				) );
		...
		$page = new WP_Router_Page ( $page_contents, $title, $template, $this->twig_template );
	}
	...
	protected function get_twig_content() {
		return $this->twig_content_template;
	}
	protected function get_twig_data() {
		return $this->twig_data;
	}
	...
	protected function error_403() {
		...
		$args = apply_filters ( 'wp_router_access_denied_args', array (
				'response' => 403 
		) );
